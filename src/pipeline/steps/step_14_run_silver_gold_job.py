"""
Step 14: Run Silver-to-Gold Job
...
class Step14RunSilverGoldJob(Step):
    """Run silver-to-gold Glue job."""
    ...
    def check_can_skip(self) -> tuple[bool, str]:
        ...
        # Check if gold parquet files already exist
        objects = s3.list_objects(S3_BUCKET, prefix="gold/", suffix=".parquet", max_keys=10)
        if objects:
            return True, f"Gold data already exists ({len(objects)}+ parquet files)"
        return False, ""
        ...
        try:
            ...
            if not glue.job_exists(job_name):
                return StepResult(
                    ...
                    message=f"Job '{job_name}' not found. Run step 13 first.",
                )
            ...
            if result.state.value == "SUCCEEDED":
                ...
                # Verify output
                s3 = S3Adapter(region=AWS_REGION)
                gold_files = s3.list_objects(S3_BUCKET, prefix="gold/", suffix=".parquet", max_keys=50)
                print(f"  Gold files created: {len(gold_files)}")
